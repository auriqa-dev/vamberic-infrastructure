...
The repository and CloudFront certificate reuse are confirmed. The London certificate and API image `d5bfe09` are also confirmed. Remaining prerequisites are the existing GitHub OIDC provider, protected `vapp` Environment setup in `auriqa-dev/vamberic-platform-api`, and release image/application readiness. `localhost:5173/`, comma-separated CORS and a full-URL Cognito domain are the explicit integration defaults. Dev will own `app.vamberic.com` and `api.vamberic.com` initially; plan any later production cutover separately. No extra human approval is needed to review or synthesize these changes.


## HVM enquiry notification email

Dev config enables `NOTIFICATION_EMAIL_ENABLED=true`, sets `NOTIFICATION_EMAIL_FROM=notifications@vamberic.com`, and sets `PRODUCT_ENQUIRY_NOTIFICATION_RECIPIENTS_JSON={"product_01m2wffbf3p9p19d3nd1s2fp3x":["notifications@vamberic.com"]}`. The API image remains `d5bfe09`.
...
Notification runtime variables and the API image remain unchanged. `NOTIFICATION_EMAIL_FROM` still controls the sender and `PRODUCT_ENQUIRY_NOTIFICATION_RECIPIENTS_JSON` controls Product recipients. The application continues to use ECS task credentials, with no long-lived keys.

After an explicitly authorised deployment and notification retry, record the result and restore resource scoping from `enquiryEmailNotifications.senderIdentities` in `config/dev.ts`: `arn:aws:ses:eu-west-2:755905325223:identity/vamberic.com` and `arn:aws:ses:eu-west-2:755905325223:identity/notifications@vamberic.com`. Those identity names remain configured for restoration but are temporarily not used by this policy. Update the diagnostic test back to exact identity assertions at the same time. This change does not deploy or send a diagnostic email.
